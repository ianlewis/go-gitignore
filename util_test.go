...
func dir(content map[string]string) (string, error) {
	...
	for _key, _content := range content {
		...
		_isdir := false
		_path := _key
		if strings.HasSuffix(_path, string(filepath.Separator)) {
			_path = strings.TrimSuffix(_path, string(filepath.Separator))
			_isdir = true
		}

		// construct the absolute path (according to the local file system)
		_abs := _dir
		_parts := strings.Split(_path, string(filepath.Separator))
		_last := len(_parts) - 1
		if _isdir {
